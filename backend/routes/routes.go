...
func Register(r *gin.Engine, deps Dependencies) {
	...
	api.POST("/auth/password/reset", authLimit, deps.Auth.ResetPassword)
	...
	api.GET("/me/stats", middleware.RequireAuth(deps.Secret, deps.DB), deps.Interactions.MyStats)
	...
	api.GET("/forum/stream", deps.Forum.ForumStream)
	...
	api.PATCH("/forum/:id", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Forum.UpdateTopic)
	...
	api.PATCH("/forum/replies/:id", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Forum.UpdateReply)
	...
	api.GET("/search", middleware.OptionalAuth(deps.Secret, deps.DB), deps.Posts.Search)
	api.GET("/feed.xml", deps.Posts.RSSFeed)
	...
	api.GET("/posts/:slug/comments/stream", deps.Community.CommentStream)
	api.POST("/posts/:slug/comments", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Community.CreateComment)
	api.POST("/posts/:slug/typing", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Community.CommentTyping)
	...
	api.PATCH("/comments/:id", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Community.UpdateComment)
	...
	api.GET("/tags/:slug", middleware.OptionalAuth(deps.Secret, deps.DB), deps.Tags.TagInfo)
	api.GET("/series/:slug", deps.Series.Detail)
	api.GET("/users/:username/series", deps.Series.AuthorSeries)
	api.POST("/series", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Series.Create)
	api.POST("/tags/:id/subscribe", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Tags.SubscribeTag)
	api.DELETE("/tags/:id/subscribe", writeLimit, middleware.RequireAuth(deps.Secret, deps.DB), deps.Tags.UnsubscribeTag)
