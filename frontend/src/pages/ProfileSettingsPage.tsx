...
export function ProfileSettingsPage() {
  ...
  useEffect(() => {
    getCurrentUser().then((current) => { setUser(current); setForm({ nickname: current.nickname, avatar: current.avatar, bio: current.bio, email: current.email ?? '' }) }).catch(() => navigate('/login'))
  }, [navigate])
