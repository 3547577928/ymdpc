import { ArrowLeft, Flag, Heart, MessageCircle, Pencil, Reply, Trash2 } from 'lucide-react'
import { useEffect, useState } from 'react'
import { Link, useNavigate, useParams } from 'react-router-dom'
import { ConfirmDialog, NoticeDialog, PromptDialog } from '../components/Dialog'
import { createForumReply, createReport, deleteForumReply, deleteForumTopic, getForumTopic, likeForumReply, likeForumTopic, unlikeForumReply, unlikeForumTopic, updateForumReply, updateForumTopic } from '../services/api'
import { useAuth } from '../services/auth'
import { usePageMeta } from '../utils/usePageMeta'
import type { ForumKind, ForumReply, ForumTopic } from '../types'
import { formatDate } from '../utils'

const kindLabels = { discuss: '讨论', share: '分享', help: '求助', rant: '吐槽' } as const
const kinds = Object.keys(kindLabels) as ForumKind[]
...
export function ForumTopicPage() {
  ...
  const [editTopicInput, setEditTopicInput] = useState({ content: '', kind: 'discuss' as ForumKind })
  ...
  const saveEditTopic = async () => {
    if (!topic || !editTopicInput.content.trim()) return
    setSaving(true)
    try {
      // images 不传：服务端保留原图
      const updated = await updateForumTopic(topic.id, { content: editTopicInput.content.trim(), kind: editTopicInput.kind })
      setTopic((current) => current ? { ...current, ...updated, liked: current.liked } : current)
      setEditingTopic(false)
    } catch (reason) { setError(reason instanceof Error ? reason.message : '编辑帖子失败') } finally { setSaving(false) }
  }
  const saveEditReply = async (reply: ForumReply) => {
    if (!editReplyInput.trim()) return
    try {
      const updated = await updateForumReply(reply.id, editReplyInput.trim())
      setReplies((current) => current.map((item) => item.id === reply.id ? { ...item, ...updated, liked: item.liked } : item))
      setEditingReply(undefined)
    } catch (reason) { setError(reason instanceof Error ? reason.message : '编辑回复失败') }
  }
